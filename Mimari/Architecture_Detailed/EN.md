...
**WebApi persistence boundary (as implemented):** The HTTP request path and `NotificationDispatcherWorker` do not use `AppDbContext` directly. Tenant resolution goes through `ITenantResolutionReader`; outbox claim and post-send completion use Application interfaces (`INotificationOutboxClaimService`, `INotificationOutboxDispatchWriter`) implemented in Persistence. Remaining direct EF in WebApi is limited to the `Program.cs` composition root and Development/E2E seed. Checklist: `Documents/api.verasaha.com_WebApi_Persistence_Cleanup_Checkpoint.md`.

**Meeting aggregate and immutability:** After a meeting is completed or cancelled, certain fields cannot be changed; domain rules require a change reason. This supports audit trail and data integrity. **Meeting notes (MVP):** A dedicated `MeetingNote` entity is deferred; MVP notes are stored in `Meeting.Description` via `PATCH /api/meetings/{id}` (`description` and `locationAddress` in the body). Updates apply only while the meeting is `Planned`; completed or cancelled meetings return **HTTP 400** via domain `GuardPlannedForMutation`. **TenantAdmin** may update any planned tenant meeting; **TenantUser** may update only meetings they are assigned to (participant `UserId` match); unassigned meetings return **HTTP 403**. `Description` length follows existing EF configuration (max 2000). **Reschedule:** only `Planned` meetings move via `POST /api/meetings/{id}/reschedule` with required `changeReason` and `Idempotency-Key`. Old/new planned dates are stored on `MeetingChangeHistory`; `GET .../history-summary` exposes the latest reschedule fields. **TenantAdmin** may reschedule; **TenantUser** may not. **Cancel:** only `Planned` meetings are cancelled via `POST /api/meetings/{id}/cancel` with required `cancellationReason`, `Idempotency-Key`, and **TenantAdmin** role. The cancellation reason is stored on `MeetingChangeHistory`; after cancel, reschedule/complete and other planned mutations are rejected by the domain (HTTP 400). Replaying the same `Idempotency-Key` is safe; a second cancel with a new key on an already cancelled meeting returns 400. **Complete:** only `Planned` meetings are completed via `POST /api/meetings/{id}/complete` with required `Idempotency-Key` and **TenantAdmin** role (no body/reason). **TenantUser** receives 403. After completion, reschedule/cancel and participant mutations are rejected by the domain (HTTP 400). Replaying the same `Idempotency-Key` returns 200 safely; a new key on an already completed meeting returns 200 via domain no-op (policy differs from cancel).

**ParticipationType (Planned / Actual):** Distinguishes whether a participant was planned in advance or added in person; reporting and notification rules apply accordingly. **Met contacts (demo slice):** There is no separate `MeetingMetContact` entity; people actually met are still `MeetingParticipant` rows flagged with `ParticipationType.Actual`. `GET/POST /api/meetings/{id}/met-contacts` and `PATCH/DELETE /api/met-contacts/{id}` reuse this model. **Write validation (application layer):** for Actual rows, display name is required; email is optional but format-validated when supplied; Planned rows still require email. **Completed or cancelled** meetings reject participant/met-contact mutations (add/update/remove) via domain `GuardPlannedForMutation` (**HTTP 400**).
...
**Outbox pattern:** Notifications are written to the NotificationOutbox table; committed in the same transaction as the domain change. A background worker reads rows and sends to FCM. Handlers/controllers do not call FCM directly; at-least-once enqueue and worker-side retry/backoff/dead-letter handling are ensured. In WebApi the worker depends only on Application claim/dispatch interfaces; claim SQL and completion rules live in Persistence (`NotificationOutboxClaimService`, `NotificationOutboxDispatchWriter`).
