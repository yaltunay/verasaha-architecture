...
**WebApi kalıcılık sınırı (uygulama durumu):** HTTP istek yolu ve `NotificationDispatcherWorker` doğrudan `AppDbContext` kullanmaz. Kiracı çözümlemesi `ITenantResolutionReader` ile yapılır; outbox claim ve gönderim sonrası tamamlama `INotificationOutboxClaimService` / `INotificationOutboxDispatchWriter` Application arayüzleri üzerinden Persistence'e delege edilir. WebApi içinde kalan doğrudan EF yalnızca `Program.cs` bileşim kökü ve Development/E2E seed içindir. Kontrol listesi: `Documents/api.verasaha.com_WebApi_Persistence_Cleanup_Checkpoint.md`.

**Meeting aggregate ve immutability:** Toplantı tamamlandıktan veya iptal edildikten sonra belirli alanlar değiştirilemez; domain kuralları ile zorunlu değişiklik nedeni (reason) istenir. Bu, denetim izi ve veri bütünlüğü için tercih edilir. **Toplantı notları (MVP):** Ayrı `MeetingNote` varlığı ertelenmiştir; MVP notlar `Meeting.Description` alanına yazılır (`PATCH /api/meetings/{id}` gövdesinde `description` ve `locationAddress`). Yalnızca `Planned` toplantılarda güncellenir; tamamlanmış/iptal edilmiş toplantılarda domain `GuardPlannedForMutation` ile **HTTP 400**. **TenantAdmin** kiracıdaki planlı toplantıları güncelleyebilir; **TenantUser** yalnızca kendisine atanmış (katılımcı `UserId` eşleşmesi) toplantılarda güncelleyebilir; atanmamış toplantı **HTTP 403**. `Description` uzunluk sınırı mevcut EF yapılandırmasıdır (max 2000). **Yeniden planlama (reschedule):** yalnızca `Planned` durumdaki toplantılar `POST /api/meetings/{id}/reschedule` ile taşınır; zorunlu `changeReason` ve `Idempotency-Key` gerekir. Eski/yeni planlanan tarih `MeetingChangeHistory` satırında saklanır; özet okuma `GET .../history-summary` ile son reschedule alanlarını döner. **TenantAdmin** yazabilir; **TenantUser** yazamaz. **İptal (cancel):** yalnızca `Planned` toplantılar `POST /api/meetings/{id}/cancel` ile iptal edilir; zorunlu `cancellationReason`, `Idempotency-Key` ve **TenantAdmin** rolü gerekir. İptal gerekçesi `MeetingChangeHistory` satırında saklanır; iptal sonrası reschedule/complete ve diğer planlı mutasyonlar domain ile reddedilir (HTTP 400). Aynı `Idempotency-Key` ile tekrar çağrı güvenli yanıt döner; farklı anahtarla zaten iptal edilmiş toplantı 400 döner. **Tamamlama (complete):** yalnızca `Planned` toplantılar `POST /api/meetings/{id}/complete` ile tamamlanır; zorunlu `Idempotency-Key` ve **TenantAdmin** rolü gerekir (gövde/reason yok). **TenantUser** 403 alır. Tamamlandıktan sonra reschedule/cancel ve katılımcı mutasyonları domain ile reddedilir (HTTP 400). Aynı `Idempotency-Key` ile tekrar çağrı güvenli 200 döner; farklı anahtarla zaten tamamlanmış toplantıda domain no-op ile 200 döner (cancel’dan farklı politika).

**ParticipationType (Planned / Actual):** Katılımcının önceden planlanmış mı yoksa fiilen (yerinde) eklenmiş mi olduğunu ayırt eder; raporlama ve bildirim kuralları buna göre uygulanır. **Görüşülen kişiler (met contacts, demo):** Ayrı `MeetingMetContact` varlığı yoktur; fiilen görüşülen kişiler de `MeetingParticipant` satırıdır ve `ParticipationType.Actual` ile işaretlenir. `GET/POST /api/meetings/{id}/met-contacts` ve `PATCH/DELETE /api/met-contacts/{id}` bu modeli paylaşır. **Yazma doğrulama (uygulama katmanı):** Actual için ad soyad zorunlu; e-posta isteğe bağlıdır, dolu ise biçim doğrulanır; planlanan katılımcı için e-posta zorunludur. **Tamamlanmış veya iptal edilmiş** toplantılarda katılımcı/görüşülen mutasyonları (ekle/güncelle/sil, domain `GuardPlannedForMutation`) **HTTP 400** ile reddedilir.
...
**Outbox pattern:** Bildirimler NotificationOutbox tablosuna yazılır; aynı transaction içinde domain değişikliği ile birlikte commit edilir. Arka planda worker satırları alır, FCM'e gönderir. Handlers/controller'lar FCM'i doğrudan çağırmaz; en az bir kez kuyruğa alınma ve worker tarafında retry/backoff/dead-letter yönetimi sağlanır. Worker WebApi'de yalnızca Application claim/dispatch arayüzlerine bağlıdır; claim SQL ve tamamlama kuralları Persistence servislerindedir (`NotificationOutboxClaimService`, `NotificationOutboxDispatchWriter`).
